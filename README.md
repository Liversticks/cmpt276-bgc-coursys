...
* Two environment variables are set:
  * LOCALDATABASE/DATABASE_URL (PostgreSQL connection string)
  * SENDGRID_API_KEY

## Known bugs

* Registration after the registration deadline is still possible. (Possible fix: add date checking during js course insertion or some sort of database table constraint)
* On narrow screens, the sidebar will clip into the main view. (Possible fix: properly add the sidebar so it doesn't sit outside the main container div)
* Using the column-sort functionality of bootstrap-tables does not sort dates properly as currently, "dates" are actually strings (so 10/xx/xx would be before 9/xx/xx).
